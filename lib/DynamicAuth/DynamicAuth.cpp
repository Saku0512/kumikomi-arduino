...
Direction DynamicAuth::waitForInput(LiquidCrystal &lcd) {
  ...
  while (true) {
    ...
    int diffX = rawX - _offsetX;
    int diffY = rawY - _offsetY;
    ...
    if (absX > THRESHOLD || absY > THRESHOLD) {
      if (absX > absY) {
        ...
        // Y軸方向の成分が大きい
        if (diffY < 0)
          current = DIR_DOWN; // ※配線によりUP/DOWN逆ならここを修正
        else
          current = DIR_UP;
      }
    } else {
    ...
    if (detected == DIR_NONE && current != DIR_CENTER) {
      ...
      // ビープ音などを鳴らすならここ
      Serial.print("Input: ");
      Serial.println(dirToString(detected));
    }
...
Direction DynamicAuth::getCurrentDirection() {
  int diffX = analogRead(_pinX) - _offsetX;
  int diffY = analogRead(_pinY) - _offsetY;
  const int THRESHOLD = 200;
